...
void setup() {//___________________________________________________________________________________________
  ...
  //【割り込み関数　初期設定】
  //カラーセンサー　割り込み関数　初期設定
  void color_1_warikomi(void);
  void color_2_warikomi(void);
  void color_3_warikomi(void);
  void color_4_warikomi(void);
  //IRセンサー方向　割り込み関数　初期設定
  void ir_10_houkou_warikomi(void);
  void ir_15_houkou_warikomi(void);
  void ir_20_houkou_warikomi(void);
  void ir_25_houkou_warikomi(void);
  void ir_30_houkou_warikomi(void);
  void ir_35_houkou_warikomi(void);
  void ir_40_houkou_warikomi(void);
  void ir_45_houkou_warikomi(void);
  //超音波センサー　割り込み関数　初期設定
  void ultrasonic_1_warikomi(void);
  //コンパスセンサー　割り込み関数　初期設定
  void compusu_1_warikomi(void);
  void compusu_2_warikomi(void);
...
void loop() {
  ...
  if (DIR >= 20) {
    ...
    //return;
    //1が白色の場合
  } else if (color_h_1 >= 240) {
    color_1_warikomi();
    Serial.println("1　白ならば");
    //return;
    //２が白色の場合
  } else if (color_h_2 >= 850) {
    color_2_warikomi();
    Serial.println("2　白ならば");
    //return;
    //３が白色の場合
  } else if (color_h_3 >= 115) {
    color_3_warikomi();
    Serial.println("3　白ならば");
    //return;
    //４が白色の場合
  } else if (color_h_4 >= 325) {
    color_4_warikomi();
    Serial.println("4　白ならば");
    //return;
    //左斜め前
  } else if ((ir_h_1_and_ir_h_2) >= 1900) {
    ir_15_houkou_warikomi();
    Serial.println("左斜め前");
    //右斜め前
  } else if ((ir_h_4_and_ir_h_1) >=  1900) {
    ir_45_houkou_warikomi();
    Serial.println("右斜め前");
    //return;
    //前
  } else if (995 <= ir_h_1 ) {
    ir_10_houkou_warikomi();
    Serial.println("前");
    //return;
    //左
  } else if (997 <= ir_h_2) {
    ir_20_houkou_warikomi();
    Serial.println("左");
    //return;
    //左斜め後ろ
  } else if ((ir_h_2_and_ir_h_3) >= 1900) {
    ir_25_houkou_warikomi();
    Serial.println("右斜め後ろ");
    //return;
    //後ろ
  } else if (995 <= ir_h_3) {
    ir_30_houkou_warikomi();
    Serial.println("後ろ");
    //return;
    //右斜め後ろ
  } else if ((ir_h_3_and_ir_h_4) >= 1900) {
    ir_35_houkou_warikomi();
    Serial.println("右斜め後ろ");
    //return;
    //右
  } else if (997 <= ir_h_4) {
    ir_40_houkou_warikomi();
    Serial.println("右");
    //return;
    //５CM以内にものがある時
  }  /*else if ((cm) <= 5) {
    ultrasonic_1_warikomi();
    Serial.println("5CM以内の時");
    //return;
    //それ以外
  } */else {
    Serial.println("それ以外");
...
//【割り込み関数　動作内容】____________________________________________________________________________________________________________
...
void compusu_1_warikomi() {
  ...
  Serial.println("x軸が20以上の場合");
  for (iii = 0; iii < 5; iii++) {
    Serial1.println("1R1003F1002R1004F100");
    delay(10);
  }
  //return;
  //}
...
void compusu_2_warikomi() {
  ...
  for (iii = 0; iii < 5; iii++) {
    Serial1.println("1F1003R1002F1004R100");
    delay(10);
  }
  //return;
  //}
